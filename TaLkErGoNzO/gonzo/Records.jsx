...
const Records = () => {
    ...
    useFocusEffect(
        useCallback(() => {
            loadFavoritesAndCustomTopics();
        }, [])
    );

    useEffect(() => {
        loadFavoritesAndCustomTopics();
    }, [favoriteTopics, customTopics]);

    const loadFavoritesAndCustomTopics = async () => {
        const storedFavs = await AsyncStorage.getItem('FAV_TOPICS');
        const storedCustom = await AsyncStorage.getItem('personalRecords');

        if (storedFavs) setFavoriteTopics(JSON.parse(storedFavs));

        if (storedCustom) {
            const customTopics = JSON.parse(storedCustom);
    ...
    const filteredTopics = customTopics.filter((item) => {
    ...
    const toggleSelectedRecord = async (item) => {
        try {
            if (playingId === item.title) {
                await audioRecorderPlayer.stopPlayer();
                setPlayingId(null);
            } else {
                await audioRecorderPlayer.startPlayer(item.record);
                audioRecorderPlayer.addPlayBackListener(() => {});
                setPlayingId(item.title);
            }
        } catch (error) {
    ...
    return (
        <Default navigation={true}>
            <View style={styles.container}>
                ...
                <Text style={styles.mainTitle}>Records</Text>
                            ...
                            <View style={{width: '100%', paddingHorizontal: 16}}>
                                {
                                    filteredTopics.map((item, idx) => (
                                        ...
                                        >
                                            <ImageBackground
                                                source={typeof item.imageName === 'string'
                                                    ? { uri: item.imageName }
                                                    : item.imageName}
                                            ...
                                            >
                                                <View style={styles.topicInnerTextConatiner}>
                                                    ...
                                                    <View  style={styles.row}>
                                                        ...
                                                        <TouchableOpacity
                                                            style={[styles.topicBtn, { backgroundColor: '#FB7401', opacity: item.record ? 1 : 0.5 }]}
                                                            onPress={() => toggleSelectedRecord(item)}
                                                        >
                                                            <Image
                                ...
                                <View style={{width: '100%', flexGrow: 1}}>
                                    <View style={styles.noTopicsBox}>
                                        <Text style={styles.noTopicsText}>There are no themes for your search, please try again.</Text>
                                    </View>
